...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task in the current branch. Do not use subagents. Do not use worktrees. Steps use checkbox (`- [x]`) syntax for tracking.
...
- [x] **Step 1: Add desktop CSS convergence tests**
...
- [x] **Step 2: Add extension CSS convergence tests**
...
- [x] **Step 3: Run the new guardrail tests and confirm they fail on the current UI**

Run:

```bash
pnpm --filter @immersive-subs/desktop-app exec vitest run src/renderer/styleConvergence.test.ts
pnpm --filter @immersive-subs/extension exec vitest run src/popup-style.test.ts
```
...
- [x] **Step 4: Normalize desktop and extension tokens**
...
- [x] **Step 5: Run guardrail tests and keep them passing**
...
- [x] **Step 2: Create `UiSegmentedControl.vue`**
...
- [x] **Step 3: Create `UiProgress.vue`**
...
- [x] **Step 4: Export the primitives**
...
- [x] **Step 5: Style the new primitives through existing token roles**
...
```bash
pnpm --filter @immersive-subs/desktop-app exec vitest run src/renderer/components/ui/UiComponents.test.ts
```
...
- [x] **Step 2: Create Vue icon components**
...
- [x] **Step 3: Export new icon components**
...
- [x] **Step 4: Use icons in top panel and playback controls**
...
```bash
pnpm --filter @immersive-subs/desktop-app exec vitest run src/renderer/components/top-panel/TopControlPanel.test.ts src/renderer/components/subtitle/SubtitleView.browser.test.ts
```
...
- [x] **Step 1: Use `UiSegmentedControl` for appearance**
...
- [x] **Step 2: Make repeated records use `UiListItem` only**
...
- [x] **Step 3: Make Faster-Whisper native to the host UI**
...
- [x] **Step 4: Remove feature-specific settings visual CSS**
...
- [x] **Step 1: Make top panel chrome solid and token-driven**
...
- [x] **Step 2: Use shared controls for all subtitle controls**
...
- [x] **Step 3: Make progress sliders plain line controls**
...
- [x] **Step 4: Tokenize word lookup chrome only**
...
- [x] **Step 5: Run control and word lookup tests**
...
- [x] **Step 1: Rename active visual classes to generic popup primitives**
...
- [x] **Step 2: Update `popup.ts` selectors and generated markup**
...
- [x] **Step 3: Make popup rows flat and line-based**
...
- [x] **Step 4: Keep appearance behavior independent**
...
- [x] **Step 5: Run extension tests and build**
...
- [x] **Step 1: Run explicit legacy visual scans**
...
rg -n --glob '!**/*.test.ts' --glob '!**/*.browser.test.ts' --glob '!**/shared/constants.ts' "class=\"[^\"]*(fw-|plugin-card|usp-|media-card|server-card)|className = \"(server-card|media-card)|id=\"media-card-template\"" apps/desktop-app/src/renderer apps/extension/popup.html apps/extension/src
```
...
- Non-visual ids, job ids, and protocol constants are not UI class matches.
...
- [x] **Step 3: Run extension verification**
...
- [x] **Step 4: Record final implementation status in this plan**

The implementation result is recorded near the top of this file.
