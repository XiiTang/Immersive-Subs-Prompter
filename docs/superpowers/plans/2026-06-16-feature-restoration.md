...
- The profile list uses the right-side small hollow circle only as non-fallback profile enablement. Clicking a profile row selects it for editing; clicking the circle toggles `enabled` without changing editor selection. Disabled profiles keep their URL rules but do not participate in runtime URL-rule matching. The fallback profile remains last and has no enablement control.
