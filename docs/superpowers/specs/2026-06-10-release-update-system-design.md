...
Scripts fail on mismatched package versions, invalid SemVer, missing required release URLs, duplicate platform keys, missing checksums, missing desktop artifacts for macOS, Windows, or Linux, and invalid extension store status.

Scripts do not preserve older draft formats or update legacy release metadata.

## Built-In Features Boundary

Built-in features ship inside the desktop app artifact. They do not have separate package artifacts, repository manifests, install URLs, update checks, or release channels.

The desktop app release manifest lists desktop artifacts and extension artifacts only. It does not list per-feature versions or per-feature update status.
...
- separate feature package release management
