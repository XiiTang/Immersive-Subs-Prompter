...
import * as gatherer from "./VideoStateGatherer";
...
describe("LoopController", () => {
  ...
  it("records A-B loop session metadata when a range loop starts", () => {
    const video = { currentTime: 8, paused: false, play: vi.fn(() => Promise.resolve()) } as unknown as HTMLVideoElement;
  ...
  it("marks boundaryTransition as loop-wrap before reporting a B-to-A jump", () => {
    const video = { currentTime: 4.2, paused: false, play: vi.fn(() => Promise.resolve()) } as unknown as HTMLVideoElement;
