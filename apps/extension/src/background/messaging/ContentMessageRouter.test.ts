...
describe("ContentMessageRouter", () => {
  ...
  it("broadcasts raw content playback samples to desktop", () => {
    vi.useFakeTimers();
  ...
  it("keeps dashboard projection separate from desktop broadcasts", () => {
    vi.useFakeTimers();
    vi.setSystemTime(5000);
    try {
      const logger = {
      ...
      expect(snapshotBuilder.buildMediaSnapshot()).toEqual([
